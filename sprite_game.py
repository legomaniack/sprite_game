...
class CharactorReplaceEvent(Event):
	def __init__(self, charactor):
		self.name = "Charactor Replace Event"
		self.charactor = charactor
	...
	def notify(self, event):
		if isinstance(event, TickEvent):
			for event in pygame.event.get():
				...
				elif event.type == KEYUP and (event.key == K_w or event.key == K_UP):
					self.stack.remove(1)
			...
			move_dir = self.stack.get_last()
			if move_dir:
				self.event_manager.post(CharactorMoveRequest(move_dir))
...
class PygameView:
	...
	def notify(self, event):
		...
		if isinstance(event, CharactorMoveEvent):
			...
			self.get_sprite(event.charactor).counter = 0
		...
		if isinstance(event, CharactorSpriteEvent):
			self.sprites[id(event.charactor)] = CharactorSprite(event.charactor)
			if event.file != None:
				self.get_sprite(event.charactor).build(event.file)
		if isinstance(event, CharactorReplaceEvent):
			self.get_sprite(event.charactor).teleport_to_pos()
...
class CharactorSprite:
	def __init__(self, charactor):
		...
		self.counter = 0
		self.direction = 3
		self.direction_to_sprite_map = [6,1,7,2,3,4,0,5]
	...
	def update(self):
		...
		if not self.is_moved():
			#set sprite
			sprite_num = 2+int((self.counter)/2)
			self.sprite = self.sprites[self.direction_to_sprite_map[self.direction]][sprite_num]
			#debug(str(self.counter))
			#debug(str(sprite_num))
			xdir, ydir = self.get_move_dir()
			self.x += xdir*4
			self.y += ydir*4
		else:
			sprite_num = int(self.counter/6)
			self.sprite = self.sprites[self.direction_to_sprite_map[self.direction]][sprite_num]
			#debug(str(sprite_num))
...
class Game:
	...
	def start(self):
		self.event_manager.post(DrawMapEvent())
		self.player.build('eevee_sprites.png')
...
class Map:
	...
	def draw_map(self):
		...
		self.load_spritesheet('beach_cave_tiles.png')
		...
		for r in range(len(self.layout)):
			...
			for c in range(len(self.layout[r])):
				new_sector = Sector(gen_map.key[self.layout[r][c]]['type'], gen_map.key[self.layout[r][c]]['walkable'], (c,r))
	...
	def notify(self, event):
		...
		if isinstance(event, SpriteMoveEvent):
			if event.charactor == self:
				self.done_moving = True
				if self.sector.sprite_type == 'stairs':
					self.event_manager.post(DrawMapEvent())
					self.event_manager.post(CharactorReplaceEvent(self))
